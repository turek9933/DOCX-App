...
#Zamiana ciągu 0123456789 na ", do zmiany kiedyś tam. TO DO TODO
#def add_quote(f_docx_read, f_docx_write):
#    
#    for line in f_docx_read:
#        temp_line = str(line)
#        str_to_find = "cudzyslow"
#        if temp_line.find(str_to_find) != -1:
#            temp_line = temp_line.replace(str_to_find, "ghgfhfg")
#            print("SKYLARRRRRRRRRRRRR")
        ...
        with fileinput.FileInput(files=(cwd + sys_id + "document_original.xml")) as f_docx_read:
            ...
            with open(cwd + sys_id + "temp" + sys_id + "word" + sys_id + "document.xml", 'w') as f_docx_write:
                ...
                for line in f_docx_read:
                    temp_line = str(line)
                    char_code_numer = 48
                    str_to_find = "cudzyslow"#TODO AAAAAAA
                    while temp_line.find(digit_word(char_code_numer)) != -1 and index < len(data_line_splited):
                        ...
                        index += 1
                    if temp_line.find(str_to_find) != -1:
                        temp_line = temp_line.replace(str_to_find, "\"")
                        print("SKYLARRRRRRRRRRRRR")
                    f_docx_write.write(temp_line)
                #add_quote(f_docx_read, f_docx_write)#TO DO TODO
